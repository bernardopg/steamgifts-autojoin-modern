...
function updatePopupState(state) {
  if (state) {
    currentPointsSpan.textContent =
      typeof state.points === "number" && state.points !== -1
        ? `${state.points}P`
        : "N/A";
    joinCountSpan.textContent =
      typeof state.joinCount === "number" ? state.joinCount : "N/A";
  } else {
      ...
      chrome.tabs.sendMessage(
        ...
        (response) => {
          if (chrome.runtime.lastError) {
            console.log(
              "Popup: Could not message content script for state.",
...
enableSwitch.addEventListener("change", () => {
  ...
  chrome.storage.sync.set({ autoJoinEnabled: isEnabled }, () => {
    if (chrome.runtime.lastError) {
      ...
      updateStatusIndicator(isEnabled);
      console.log("Indicator enabled state saved:", isEnabled);
    }
  });
});
...
joinAllVisibleBtn.addEventListener("click", () => {
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    if (tabs[0]?.id && tabs[0].url?.includes("steamgifts.com")) {
      chrome.tabs.sendMessage(
        tabs[0].id,
        { action: "joinAllVisible" },
        (response) => {
          if (chrome.runtime.lastError) {
            console.error(
              "Error sending joinAllVisible:",
              chrome.runtime.lastError.message
            );
          ...
          setTimeout(() => {
            ...
            joinAllVisibleBtn.innerHTML =
              '<i class="fa fa-bolt"></i> Join All Visible & Eligible';
            fetchPopupState(); // Refresh counts after attempting
          }, 1500);
        }
      );
    } else {

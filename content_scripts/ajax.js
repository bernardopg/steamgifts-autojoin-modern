...
export async function attemptAjaxJoin(
  code,
  cost,
  giveawayElement,
  buttonElement = null
  ...
  console.log(
    `  -> ${joinType} Attempting AJAX join for code: ${code} (Cost: ${cost}P)`
  );
  ...
  try {
    ...
    if (!response.ok) {
      ...
      success = true;
      State.incrementSessionJoinCount();
      ...
      if (currentPoints !== null && currentPoints !== -1 && cost !== null) {
        ...
        console.log(
          `  -> Locally updated points estimate: ${State.getPointsAtStart()}P`
        );
      }
      const pointsDisplay = document.querySelector(Selectors.pointsSelector);
      if (
        pointsDisplay &&
        State.getPointsAtStart() !== null &&
        State.getPointsAtStart() !== -1
      ) {
        pointsDisplay.textContent = `${State.getPointsAtStart()}P`;
      }
    } else {
      console.warn(
        `  -> AJAX Non-Success. Type: ${result.type}. Msg: ${
          result.msg || "N/A"
        }`
      );
    ...
    if (success || result?.msg?.toLowerCase().includes("already entered")) {
      ...
      if (errorMessage.toLowerCase().includes("insufficient points")) {
        btnErrorText = '<i class="fa fa-coins"></i> Points?';
      } else if (
        errorMessage.toLowerCase().includes("ended") ||
        errorMessage.toLowerCase().includes("exist")
      ) {
        btnErrorText = '<i class="fa fa-times-circle"></i> Ended?';
      }
    ...
    if (buttonElement && !success && buttonElement.style.display !== "none") {
      ...
      buttonElement.classList.remove("disabled");
      if (
        !buttonElement.innerHTML.includes("Error") &&
        !buttonElement.innerHTML.includes("Points?") &&

...
async function handleManualJoinClick(event) {
  ...
  if (
    ...
    isNaN(cost) ||
    !giveawayRow ||
    !giveawayRow.classList.contains(
      Selectors.listView_giveawaySelector.replace(".", "")
    )
  ) {
    console.error(
      "Manual Join Error: Missing data or failed to find correct parent row.",
      {
        giveawayCode,
        cost,
        parentElementTag: giveawayRow?.tagName,
        expectedClass: Selectors.listView_giveawaySelector.replace(".", ""),
        actualClasses: giveawayRow?.classList?.toString(),
  ...
  if (currentPoints === null) {
    alert("Could not determine points. Please refresh.");
    return;
  }
  if (!xsrfToken) {
  ...
  if (currentPoints < cost + State.getPointBuffer()) {
    alert(
      `Cannot join: Insufficient points.\nNeed: ${
        cost + State.getPointBuffer()
      } (Cost: ${cost} + Buffer: ${State.getPointBuffer()})\nHave: ${currentPoints}`
    );
    return;
  }
  ...
  if (isWhitelisted) {
    console.log(
      "  -> Whitelisted item found, bypassing some filters for manual join."
    );
  }

  if (!isWhitelisted && cost < State.getMinCost()) {
    alert(
      `Cannot join: Cost (${cost}P) is below your minimum setting (${State.getMinCost()}P).`
    );
    return;
  }
  if (!isWhitelisted && cost > State.getMaxCost()) {
    alert(
      `Cannot join: Cost (${cost}P) is above your maximum setting (${State.getMaxCost()}P).`
    );
    return;
  }
  ...
  if (requiredLevel > State.getMaxLevel()) {
    alert(
      `Cannot join: Requires Level ${requiredLevel}, your maximum is ${State.getMaxLevel()}.`
    );
    return;
  }
  const isGroup = !!giveawayRow.querySelector(
    Selectors.listView_groupIndicatorSelector
  );
  if (!isWhitelisted && State.shouldSkipGroups() && isGroup) {
    alert(`Cannot join: You have chosen to skip group giveaways.`);
    return;
  }
  const isWishlistedOnPage = !!giveawayRow.querySelector(
    Selectors.listView_wishlistIndicatorSelector
  );
  if (!isWhitelisted && State.isWishlistOnly() && !isWishlistedOnPage) {
    alert(`Cannot join: You have chosen to only join wishlist giveaways.`);
    return;
  }
  if (Utils.isBlacklisted(giveawayRow)) {
    alert(`Cannot join: Game title is blacklisted in your settings.`);
    return;
  }
  if (Utils.isOwned(giveawayRow)) {
    alert(
      `Cannot join: Game is marked as owned in your library (based on page indicator).`
    );
    return;
  }
...
function addOrUpdateButton(
  ...
  let button = giveawayElement.querySelector(
...
export function processListViewToAddButtons() {
  ...
  for (const giveawayElement of giveawayElements) {
    ...
    const existingButton = giveawayElement.querySelector(
      `.${Selectors.manualJoinButtonClass}`
    );
    ...
    const isWishlisted = !!giveawayElement.querySelector(
      Selectors.listView_wishlistIndicatorSelector
    );
    ...
    if (isBlacklisted) {
      passesFilters = false;
      filterReason = "Filtered: Blacklisted Keyword";
    } else if (isOwned) {
      passesFilters = false;
      filterReason = "Filtered: Game Owned";
    } else if (requiredLevel > State.getMaxLevel()) {
      passesFilters = false;
      filterReason = `Filtered: Requires Level ${requiredLevel} (Max: ${State.getMaxLevel()})`;
    } else if (!isWhitelisted) {
      if (State.shouldSkipGroups() && isGroup) {
        passesFilters = false;
        filterReason = "Filtered: Group Giveaway";
      } else if (State.isWishlistOnly() && !isWishlisted) {
        passesFilters = false;
        filterReason = "Filtered: Not on Wishlist";
      } else if (cost > State.getMaxCost()) {
        passesFilters = false;
        filterReason = `Filtered: Cost ${cost}P > Max Cost ${State.getMaxCost()}P`;
      } else if (cost < State.getMinCost()) {
        passesFilters = false;
        filterReason = `Filtered: Cost ${cost}P < Min Cost ${State.getMinCost()}P`;
      }
    }

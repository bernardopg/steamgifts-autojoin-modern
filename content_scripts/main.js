...
async function processListViewForAutoJoin() {
    ...
    if (!State.getXsrfToken()) {
        ...
        State.setIsProcessing(false); return;
    }
    ...
    for (const giveawayElement of giveawayElements) {
        ...
        if (giveawayCode) {
            console.log(`  -> Auto-joining eligible giveaway (Code: ${giveawayCode}, Cost: ${cost}P)`);
            joinAttemptedThisCycle = true;
            const success = await attemptAjaxJoin(giveawayCode, cost, giveawayElement, null);

            if (success) { console.log("  -> Auto-join successful."); }
            else { console.log("  -> Auto-join attempt failed or resulted in non-success."); }
...
async function joinAllVisibleGiveaways() {
     if (State.isProcessing()) { alert("Already processing, please wait."); return; }
     console.log("[SG AutoJoin] Join All Visible requested...");
     State.setIsProcessing(true);
     State.setXsrfToken(Utils.getXsrfTokenFromPage());
     if (!State.getXsrfToken()) { alert("Could not find security token (XSRF). Please refresh."); State.setIsProcessing(false); return; }
     const giveawayElements = document.querySelectorAll(Selectors.listView_giveawaySelector);
     let joinedCount = 0, skippedCount = 0, failedCount = 0;
     for (const giveawayElement of giveawayElements) {
         const isAlreadyJoined = giveawayElement.classList.contains(Selectors.listView_alreadyJoinedClass) || giveawayElement.parentElement?.classList.contains(Selectors.listView_alreadyJoinedClass);
         if (isAlreadyJoined) continue;
         if (Utils.isBlacklisted(giveawayElement)) { skippedCount++; continue; }
         if (Utils.isOwned(giveawayElement)) { skippedCount++; continue; }
         const requiredLevel = Utils.extractRequiredLevel(giveawayElement);
         if (requiredLevel > State.getMaxLevel()) { skippedCount++; continue; }
         const cost = Utils.extractCost(giveawayElement);
         if (cost === null) { skippedCount++; continue; }
         const isWhitelisted = Utils.isWhitelisted(giveawayElement);
         if (!isWhitelisted) {
             const isGroup = !!giveawayElement.querySelector(Selectors.listView_groupIndicatorSelector);
             if (State.shouldSkipGroups() && isGroup) { skippedCount++; continue; }
             const isWishlistedOnPage = !!giveawayElement.querySelector(Selectors.listView_wishlistIndicatorSelector);
             if (State.isWishlistOnly() && !isWishlistedOnPage) { skippedCount++; continue; }
             if (cost > State.getMaxCost()) { skippedCount++; continue; }
             if (cost < State.getMinCost()) { skippedCount++; continue; }
         }
         const currentPoints = Utils.getCurrentPoints();
         if (currentPoints === null || currentPoints < cost + State.getPointBuffer()) { skippedCount++; continue; }
         const linkElement = giveawayElement.querySelector(Selectors.listView_giveawayLinkSelector);
         const giveawayUrl = linkElement?.href;
         const giveawayCode = giveawayUrl ? Utils.extractGiveawayCodeFromUrl(giveawayUrl) : null;
         if (giveawayCode) {
...
export function runCheck() {
  chrome.storage.sync.get(
    ...
    (items) => {
      if (chrome.runtime.lastError) { console.error("Error fetching settings:", chrome.runtime.lastError); clearTimers(); return; }
...
function initialize() {
    ...
    chrome.storage.onChanged.addListener((changes, areaName) => {
        if (areaName === 'sync') {
            if (changes.autoJoinEnabled) { State.setAutoJoinEnabled(!!changes.autoJoinEnabled.newValue); updateIndicatorUI(State.isAutoJoinEnabled()); }
            if (changes.autoModeEnabled) State.setAutoModeEnabled(!!changes.autoModeEnabled.newValue);
